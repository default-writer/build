...
namespace Build
{
    ...
    {
        const string constructorType = ":()";
        const string parameterType = ":({0})";
        ...
        RuntimeType this[string typeId, RuntimeType type, string subtype]
        {
            get
            {
                if (!_types.ContainsKey(typeId + subtype))
                    _types.Add(typeId + ":base", type);
                _types[typeId + ":base"].RegisterType(type.Type);
                return _types[typeId + ":base"];
            }
        }
        ...
        {
            if (_types.ContainsKey(typeId + ":base"))
                return _types[typeId + ":base"].CreateInstance(_types[typeId + ":base"], args);
        ...
        void RegisterConstructor(Type type, ConstructorInfo constructor)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                var parameterRuntimeType = this[typeFullName, parameterRuntimeTypeBase, i];
        ...
        void RegisterConstructorType(ConstructorInfo constructor, Type type, RuntimeType baseRuntimeType)
        {
            ...
            var injectedType = (RuntimeType)_typeParser.Find(typeId, parameterArgs, _types.Values);
            string typeFullName = _typeResolver.GetTypeFullName(injectedType, parameterArgs, typeId);
            if (_types.ContainsKey(typeFullName + ":base") && this[typeFullName, _types[typeFullName + ":base"]].IsInitialized)
                return;
