...
namespace Build
{
    public class TypeBuilder
    {
        ...
        {
            var constructorTypeFullName = string.Format("{0}({1})", type.FullName, string.Join(",", constructor.GetParameters().Select(p => p.ParameterType.FullName).ToArray()));
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                var subtype = string.Format("{0}:({1})", constructorTypeFullName, i);
                parameterRuntimeType.Attribute.RegisterRuntimeType(subtype, attribute);
                ...
                {
                    if (!parameterRuntimeType.RegisterParameters(parameterRuntimeType.Attribute, constructorTypeFullName, attribute.Args))
