...
namespace Build
{
    ...
    class RuntimeType : IRuntimeType
    {
        ...
        bool _init;
        bool _guard;
        Func<IRuntimeAttribute, object> _func;
        ...
        object Evaluate(IRuntimeAttribute type) => Activator.CreateInstance(_type, _args.Select((p, i) => p.Create(type, Id, i)).ToArray());
        object Call(IRuntimeAttribute type) => Activator.CreateInstance(_type, _args.Select((p, i) => p[type, Id, i]).ToArray());
        object Create(IRuntimeAttribute type, string typeId, int? i)
        {
            object Evaluate()
            {
                if (_guard)
                ...
                _guard = true;
                object result = _func(this._attribute);
                _guard = false;
                return result;
            }
            switch (_runtimeInstance)
            {
                case RuntimeInstance.Singleton:
                    if (!_guard)
                    {
                        this[type, Id, i] = Evaluate();
                        _guard = true;
                    }
                    return this[type, Id, i];
        ...
        public object CreateInstance(IRuntimeType type, params object[] args)
        {
            ...
            if (args.Length == 0)
                return Create(type.Attribute, Id, null);
            return Call(type.Attribute);
        }
