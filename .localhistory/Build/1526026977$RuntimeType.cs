...
namespace Build
{
    ...
    class RuntimeType : IRuntimeType
    {
        ...
        public object this[IRuntimeAttribute attribute, int i]
        {
            get
            {
                if (!_values.ContainsKey(attribute.GetRuntimeType(i.ToString())))
                    _values.Add(attribute.GetRuntimeType(i.ToString()), null);
                return _values[attribute.GetRuntimeType(i.ToString())];
            }
            set
            {
                Debug.WriteLine("{0} Attribute {1} Value {2}", this, attribute, value);
                _values[attribute.GetRuntimeType(i.ToString())] = value;
            }
        }
