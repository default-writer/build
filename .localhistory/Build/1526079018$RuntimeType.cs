    ...
    partial class RuntimeType : IRuntimeType
    {
        ...
        object Create(IRuntimeType type, IRuntimeAttribute attribute, string typeId, int? i)
        {
            object Evaluate()
            {
                ...
                _guard = true;
                object result = null;
                IRuntimeAttribute runtimeAttribute = i.HasValue ? attribute.GetRuntimeType(string.Format("{0}:({1})", type.Id, i)) : null;
            ...
            if (_args != null && args != null && _args.Count == args.Length)
                for (int i = 0; i < args.Length; i++)
                {
                    ...
                    if (args[i] != null && !parameterType.IsAssignableFrom(runtimeType))
                        return false;
                    //IRuntimeAttribute runtimeAttribute = attribute.GetRuntimeType(string.Format("{0}:({1})", typeFullName, i));
                    //var injection = runtimeAttribute as InjectionAttribute;
                    //if (injection != null && injection.Args.Length > 0)
                    //    injection.Args[i] = args[i];
