...
namespace Build
{
    ...
    class RuntimeType : IRuntimeType
    {
        ...
        public object this[IRuntimeAttribute attribute]
        {
            get
            {
                if (!_values.ContainsKey(attribute))
                    _values.Add(attribute, null);
                return _values[attribute];
            }
            set
            {
                Debug.WriteLine("{0} Value {1}", this, value);
                _values[attribute] = value;
            }
        }
        object Evaluate(RuntimeType type) => Activator.CreateInstance(_type, _args.Select(p => p.Create(type)).ToArray());
        object Call(RuntimeType type) => Activator.CreateInstance(_type, _args.Select(p => p[type.Attribute]).ToArray());
        ...
        public object CreateInstance(IRuntimeType type, params object[] args)
         {
            ...
            if (args.Length == 0)
                return Create((RuntimeType)type);
