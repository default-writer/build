...
namespace Build
{
    public class TypeBuilder
    {
        ...
        RuntimeType this[string typeId, RuntimeType type, string subtype]
        {
            get
            {
        ...
        void RegisterConstructor(Type type, ConstructorInfo constructor)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                var parameterRuntimeType = this[typeFullName, parameterRuntimeTypeBase, string.Format(Parameter, "")];
                parameterRuntimeTypeBase.Attribute.RegisterRuntimeType(parameterRuntimeType);
                ...
                {
                    if (!parameterRuntimeType.RegisterParameters(attribute, attribute.Args))
                        throw new TypeRegistrationException(string.Format("{0} is not registered (parameters mismatch)", type.FullName));
                    if (!parameterRuntimeTypeBase.RegisterParameters(parameterRuntimeType.Attribute, attribute.Args))
                        throw new TypeRegistrationException(string.Format("{0} is not registered (parameters mismatch)", type.FullName));
                }
        ...
        void RegisterConstructorType(ConstructorInfo constructor, Type type, RuntimeType baseRuntimeType)
        {
            ...
            var injectedType = (RuntimeType)_typeParser.Find(typeId, parameterArgs, _types.Values);
            string typeFullName = _typeResolver.GetTypeFullName(injectedType, parameterArgs, typeId);
            if (_types.ContainsKey(typeFullName + subtype) && this[typeFullName, _types[typeFullName + subtype], subtype].IsInitialized)
                return;
            var runtimeType = this[typeFullName, baseRuntimeType, subtype];
            if (runtimeType != null)
            {
