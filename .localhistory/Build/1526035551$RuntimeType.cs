...
namespace Build
{
    ...
    class RuntimeType : IRuntimeType
    {
        ...
        object Call(RuntimeType type) => Activator.CreateInstance(_type, _args.Select((p, i) => p[type.Attribute, type.Id, i]).ToArray());
        ...
        public bool RegisterParameters(IRuntimeAttribute attribute, string typeId, params object[] args)
        {
            ...
            if (_args != null && args != null && _args.Count == args.Length)
                for (int i = 0; i < args.Length; i++)
                {
                    ...
                    _args[i][attribute, typeId, i] = args[i];
                }
            return true;
        }
        //public object GetValue(RuntimeType type) => _values[type.Attribute];
        public override string ToString() => string.Format("Runtime {0}({1}) Attribute {2}", _type.FullName, string.Join(",", _args.Select(p => p.RuntimeTypeId)), _attribute);
        public string Id => string.Format("{0}({1})", _type.FullName, string.Join(",", _args.Select(p => p.RuntimeTypeId)));
