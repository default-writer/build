...
namespace Build
{
    ...
    class RuntimeType : IRuntimeType
    {
        ...
        object Evaluate(RuntimeType type) => Activator.CreateInstance(_type, _args.Select((p, i) => p.Create(this, Id, i)).ToArray());
        ...
        public string RuntimeTypeId => string.Format("{0}({1})", _type.FullName, string.Join(",", _args.Select(p => p.Type.FullName).ToArray()));
        public string Id => string.Format("{0}({1})", _type.FullName, _args.Select(p => p._types[0].FullName));
